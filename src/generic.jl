...
# Returns
	...
	- `mapping_per_col_level`: Maps each level for each column in a subset of the categorical columns of
...
"""
Given a table `X` and a dictionary `mapping_per_col_level` which maps each level for each column in 
a subset of categorical columns of X into a scalar or a vector (as specified in single_col)

  - transforms each value (some level) in each column in `X` using the function in `mapping_per_col_level` 
  into a scalar (single_col=true)

  - transforms each value (some level) in each column in `X` using the function in `mapping_per_col_level` 
  into a set of k columns where k is the length of the vector (single_col=false)
	...
	for col in col_names
		...
		if col in keys(mapping_per_col_level)
			if single_col
				...
				col_names_with_inds = generate_new_column_names(
					col,
					length(first(mapping_per_col_level[col])[2]),
				)
