...
@testset "Ordinal Encoding Fit" begin
    ...
    X = dataset_forms[1]
    result = ordinal_encoder_fit(X)[:index_given_col_level]
...
@testset "MLJ Interface Ordinal Encoding" begin
    ...
    for X in Xs
        # functional api
        fit_res = ordinal_encoder_fit(X; exclude_cols=true, encode_ordinal=false)
        X_transf = ordinal_encoder_transform(X, fit_res)
        # mlj api
        encoder = OrdinalEncoder( exclude_cols=true, encode_ordinal=false)
        ...
        @test fitresult.index_given_col_level == fit_res[:index_given_col_level]
